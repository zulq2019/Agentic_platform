# PI-03-Orchestrator — AI Implementation Prompts

Use these prompts when working with AI assistants to implement Orchestrator.
Always attach CLAUDE.md and the relevant contract schemas as context.

## PROMPT-PI-03-Orchestrator-01 — Core Service Implementation

```
Context files: CLAUDE.md, contracts/event-envelope.schema.json, ARCHITECTURE.md

Implement the Orchestrator services for the Agentic Engineering Platform.

Services: orchestrator-service, workflow-engine, task-engine, approval-service
...
## PROMPT-PI-03-Orchestrator-02 — Integration Test

```
Context files: CLAUDE.md, existing test files in tests/integration/

Write an integration test for the primary event flow introduced in Orchestrator.
...
## PROMPT-PI-03-Orchestrator-03 — Observability

```
Add OpenTelemetry instrumentation to all Orchestrator services.
