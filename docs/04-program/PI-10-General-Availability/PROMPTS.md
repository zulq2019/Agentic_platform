# PI-10-General-Availability — AI Implementation Prompts

Use these prompts when working with AI assistants to implement General Availability.
Always attach CLAUDE.md and the relevant contract schemas as context.

## PROMPT-PI-10-General-Availability-01 — Core Service Implementation

```
Context files: CLAUDE.md, contracts/event-envelope.schema.json, ARCHITECTURE.md

Implement the General Availability services for the Agentic Engineering Platform.

Services: Terraform + K8s + Helm + full observability stack
...
## PROMPT-PI-10-General-Availability-02 — Integration Test

```
Context files: CLAUDE.md, existing test files in tests/integration/

Write an integration test for the primary event flow introduced in General Availability.
...
## PROMPT-PI-10-General-Availability-03 — Observability

```
Add OpenTelemetry instrumentation to all General Availability services.
