...
async def test_health_ready_returns_200_when_dependencies_ok(app):
    with (
        patch("memory_service.main.check_postgres", new=AsyncMock(return_value="ok")),
        patch("memory_service.main.check_kafka", new=AsyncMock(return_value="ok")),
        patch("memory_service.main.check_redis", new=AsyncMock(return_value="ok")),
    ):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
...
async def test_health_ready_returns_503_when_database_down(app):
    with (
        patch("memory_service.main.check_postgres", new=AsyncMock(return_value="error")),
        patch("memory_service.main.check_kafka", new=AsyncMock(return_value="ok")),
        patch("memory_service.main.check_redis", new=AsyncMock(return_value="ok")),
    ):
        transport = ASGITransport(app=app)
