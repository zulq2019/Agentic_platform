...
async def test_health_ready_returns_200_when_dependencies_ok(app):
    with (
        patch("agent_registry.main.check_postgres", new=AsyncMock(return_value="ok")),
        patch("agent_registry.main.check_kafka", new=AsyncMock(return_value="ok")),
        patch("agent_registry.main.check_redis", new=AsyncMock(return_value="ok")),
    ):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
...
async def test_health_ready_returns_503_when_database_down(app):
    with (
        patch("agent_registry.main.check_postgres", new=AsyncMock(return_value="error")),
        patch("agent_registry.main.check_kafka", new=AsyncMock(return_value="ok")),
        patch("agent_registry.main.check_redis", new=AsyncMock(return_value="ok")),
    ):
        transport = ASGITransport(app=app)
