...
async def test_health_ready_returns_200_when_dependencies_ok(app):
    with (
        patch("model_router.main.check_postgres", new=AsyncMock(return_value="ok")),
        patch("model_router.main.check_kafka", new=AsyncMock(return_value="ok")),
        patch("model_router.main.check_redis", new=AsyncMock(return_value="ok")),
    ):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
...
async def test_health_ready_returns_503_when_database_down(app):
    with (
        patch("model_router.main.check_postgres", new=AsyncMock(return_value="error")),
        patch("model_router.main.check_kafka", new=AsyncMock(return_value="ok")),
        patch("model_router.main.check_redis", new=AsyncMock(return_value="ok")),
    ):
        transport = ASGITransport(app=app)
...
async def test_info_endpoint_returns_metadata(app):
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["service"] == "model-router"
